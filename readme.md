...
[Bark](https://github.com/Finb/bark) SDK for TypeScript，无需服务端，直连 Apple Push Notification service。
...
// 推送通知
await client.push({ title: 'Hello', body: '来自 bark-it 的问候' })

// 更新通知（需传入相同 id）
await client.update({ id: 'xxx', title: '更新后的标题' })

// 删除通知
await client.delete({ id: 'xxx' })
```
...
### `client.push(barkOptions, apnsOptions?, authKey?)`

发送通知，返回 `Promise<PushResult[]>`。

### `client.update(barkUpdateOptions, apnsOptions?, authKey?)`

更新已有通知，需传入 `id`。参数同 `push`，但 `id` 必填。

### `client.delete(deleteOptions, authKey?)`

删除已有通知，需传入 `id`。Bark v1.5.2 以上支持。

```ts
await client.delete({ id: 'notification-id' })
// 按设备指定
await client.delete({ id: 'notification-id', deviceToken: 'specific-token' })
```
